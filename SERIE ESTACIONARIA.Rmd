...
\  
\  
\ Se define un proceso estocástico como una familia de variables aleatorias que corresponden a momentos sucesivos del tiempo. Será designado por $Y_(t,u)$ donde $t$ es el timepo y $u$ es la variable aleatoria.  
...
* El proceso puramente aleatorio $$Y_t=e_t$$ donde $e_t$ satisface las siguientes propiedades: 
$$E[e_t]=0$$ $$E[e_t]^2=sigma^2$$ $$E[e_t,e_t]=0$$  
En el tratamiento de series temporales, se suele designar a este proceso con la denominacion de "ruido blanco". De ahora en adelante, por $e_t$ se designa únicamente a una variable aleatoria que goza de dichas propiedades
...
\ La ecuación de Dickey y Fuller es $y_t-y_{t-1}=(\phi_1-1)y_{t-1}+\epsilon_t$ donde se testea la hipotesis nula de que $\phi_1-1=0$ que podria ser contrastada respecto de la alternativa $\phi_1-1<0$, se utiliza el estdístico Tau.
\ Dickey y Fuller demostraron que la definicion de la zona crítica depende, ademas del tamaño de la muestra, de la forma de la ecuación autorregresiva (por ejemplo, si incluye o no término independente o términos de tendencia determinístca) y definieron tres clases de valores críticos para ser utilizados en los casos que:

+ $\tau$: incluya término independiente ni término lineal
...
\  La determinación mas apropiada del orden del modelo se puede lograr con el uso de criterios objetivos de selección de modelos tales como Akaike (AIC), Schwarz (BIC) o Hannan-Quinn (HQ). Este análisis procede con AIC como criterio seleccionado.  
\    
...
* *Test de normalidad de Jarque-Bera*: Pone a prueba la normalidad de los residuos. Bajo la hipótesis nula $\epsilon_t~Normal()$ contra una alternativa no específica. 
...
\    
\    
\  Una vez que se seleccionó y validó el modelo, la **fase de predicción** consiste en en utiilzar este modelo en la predicción de valores futuros de la variable objeto de estudio.
\  Dado un proceso ARMA $\phi(L)Y_t=\theta(L)\epsilon_t$ el valor que se trata predecir se expresa para el valor de $Y_{T+l}$ de la siguiente forma:
...
Analisis_DescriptivoA<-c(minimoA,maximoA,percentil1A,medianaA,percentil3A,asimetriaA,kurtosisA, promedioA, varianzaA, desvioA)
Analisis_DescriptivoB<-c(minimoB,maximoB,percentil1B,medianaB,percentil3B,asimetriaB,kurtosisB, promedioB, varianzaB, desvioB)
nombres<-c("Minimo","Maximo","Percentil 0.25","Mediana","Percentil 0.75","Coef Asimetria","Coef Curtosis", "Promedio", "Varianza", "Desvio")
names(Analisis_DescriptivoA)<-nombres
names(Analisis_DescriptivoB)<-nombres

knitr::kable(Analisis_DescriptivoA,align="l")
knitr::kable(Analisis_DescriptivoB,align="l")

```

###Fase de identificación

\ Se grafican la series para una primera impresión
  
```{r, echo=FALSE}
plot(SerieA)
plot(SerieB)
boxplot(SerieA, main= "Grafico de BoxPlot Serie A", col = "red", horizontal = T)
boxplot(SerieB, main= "Grafico de BoxPlot Serie B", col = "red", horizontal = T)
```

\ El gráfico de ambas series permite observar una reversion a la media (posiblemente cero). Se ve que ocurre con la FAC y FACP estimada,
...
acf(SerieB,main="FAS Serie B",type="covariance", col = c("blue", "orange"))
acf(SerieB,main="FAC Serie B", col = c("blue", "orange"))
pacf(SerieB,main="FACP Serie B", col = c("blue", "orange"))
```

\ Para la serie A, el gráfico de la FAC muestra un decrecimiento sinusoidal. En cuanto a la serie B, a través de la FACP se verifica que decrece lentamente de manera sinusoidal. Por otro lado, la FAC se anula al primer rezago. Los gráficos proponen la posibilidad de que la serie B sea un proceso de medias moviles no estacionaro. Para confirmar estas intuiciones son realizados los correspondientes tests de Dickey-Füller con el objetivo de determinar la existencia o no de raíces unitarias.
...
```{r, echo=FALSE}
t_empiricosA<-c(none.dfA@teststat,drift.dfA@teststat[1,1],trend.dfA@teststat[1,1])
names(t_empiricosA)<-c("None","Con Drift","Con Trend")
t_empiricosB<-c(none.dfB@teststat,drift.dfB@teststat[1,1],trend.dfB@teststat[1,1])
names(t_empiricosB)<-c("None","Con Drift","Con Trend")

knitr::kable(t_empiricosA)
knitr::kable(t_empiricosB)
...
d<-c(none.dfB@cval[1,1:3])
e<-c(drift.dfB@cval[1,1:3])
f<-c(trend.dfB@cval[1,1:3])
tau_critA<-rbind(a,b,c)
tau_critB<-rbind(d,e,f)
rownames(tau_critA)<-c("None","Con Drift","Con Trend")
rownames(tau_critB)<-c("None","Con Drift","Con Trend")

knitr::kable(tau_critA)
knitr::kable(tau_critB)
```
...
\ Para la Serie A, el modelo que mejor se ajusta segun el criterio BIC es un ARIMA (2,1,1). Para la Serie B, el que mejor se ajusta segun BIC es el modelo ARIMA (0,1,2)
...
\ Se ingresan los modelos estimado como input y el codigo devuelve
...
\  Como los *p-value* de ambas series son considerablemente mayores a un $\alpha$ de 0.1 no se rechaza la hipotesis nula de normalidad de los reisudos.
\  
...
\    
\  La primer serie resultó ser no estacionaria. Se modelizó la misma para un ARIMA(2,1,1) donde se verificaron todos los supuestos vistos en el curso y finalmente concluyó con una predicción para diez períodos.  
\  
